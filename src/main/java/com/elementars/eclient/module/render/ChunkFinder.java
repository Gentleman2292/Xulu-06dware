...
public class ChunkFinder extends Module {
    private final Value<Integer> yOffset = register(new Value<>("Y Offset", this, 0, 0, 256));
    private final Value<Boolean> relative = register(new Value<>("Relative", this, true));
    private final Value<Boolean> saveNewChunks = register(new Value<>("Save New Chunks", this, false));
    private final Value<String> saveOption = register(new Value<>("Save Option", this, "extraFolder", new ArrayList<>(
            Arrays.asList("extraFolder", "liteLoaderWdl", "nhackWdl")
    )));
    private final Value<Boolean> saveInRegionFolder = register(new Value<>("In Region", this, false));
    private final Value<Boolean> alsoSaveNormalCoords = register(new Value<>("Save Normal Coords", this, false));

    public ChunkFinder() {
        super("ChunkFinder", "Finds new chunks", Keyboard.KEY_NONE, Category.RENDER, true);
    }
    ...
    private Path getPath() {
        ...
        */
        ...
        try {
            ...
            e.printStackTrace();
            System.err.print("some exception happened when trying to make the file -> " + e.getMessage());
    ...
    private Path makeMultiplayerDirectory(){
        ...
        String folderName;
        if (saveOption.getValue().equalsIgnoreCase("liteLoaderWdl")) {
            folderName = mc.getCurrentServerData().serverName;

            rV = new File(rV, "saves");
            rV = new File(rV, folderName);
        }
        else if (saveOption.getValue().equalsIgnoreCase("nhackWdl")) {
    ...
    @EventTarget
    private void onUnload(UnloadChunkEvent event) {
    ...
    private class LastSetting {
        ...
        public boolean testChange() {
            ...
            if (!saveOption.getValue().equals(lastSaveOption)) {
                return true;
            }
